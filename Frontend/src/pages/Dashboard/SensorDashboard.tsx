...
const SensorDashboard: React.FC = () => {
  ...
  useEffect(() => {
    // Use Vite's import.meta.env to read env variables
    ...
    channel.bind("sensor-update", (data: any) => {
      console.log("Received sensor update:", data);
      
      // If the payload has a "child_node_id", update child node data
      if (data.child_node_id) {
        ...
        // Otherwise, assume it's a base node update
        setBaseNodeData({
    ...
    channel.bind("pusher:subscription_succeeded", () => {
      console.log("Pusher subscription succeeded for sensor-AGRI_001");
    });
  ...
  return (
    <div className="p-4">
      <Tab.Group
        ...
        onChange={(index) => setActiveTab(index === 0 ? 'base' : 'child')}
      >
        ...
        <Tab.Panels>
          <Tab.Panel>
            ...
            <div className="mt-4">
              ...
              <ChartOne series={[
                { name: 'Humidity', data: [baseNodeData.humidity ?? 0] },
                { name: 'Temperature', data: [baseNodeData.temperature ?? 0] }
              ]} />
            </div>
            <div className="mt-4">
